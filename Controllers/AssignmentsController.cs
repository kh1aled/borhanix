...
public class AssignmentsController(
    ...
    IAssignmentAccessService accessService) : Controller
{
    public async Task<IActionResult> Details(int id)
    {
        ...
        AssignmentAccess? myAccess = null;
        AssignmentSubmission? mySubmission = null;
        var canSubmit = false;
        var studentStatus = AssignmentStudentStatus.Active;

        if (User.IsInRole(AppRoles.Student) && userId is not null && await IsApprovedStudentAsync(assignment.CourseId, userId))
        {
            mySubmission = assignment.Submissions.FirstOrDefault(x => x.StudentId == userId);
            myAccess = await accessService.EnsureAccessAsync(assignment, userId);
            await accessService.RefreshStatusAsync(myAccess, mySubmission);

            studentStatus = mySubmission is not null
                ? AssignmentStudentStatus.Submitted
                : myAccess.Status;

            canSubmit = mySubmission is null
                && studentStatus == AssignmentStudentStatus.Active
                && DateTimeOffset.UtcNow <= myAccess.PersonalDeadlineAt;
        }

        var missingStudents = canGrade
            ? assignment.StudentAccess
                .Where(x => x.Status == AssignmentStudentStatus.Missing)
        ...
        return View(new AssignmentDetailsViewModel
        {
            ...
            PersonalDeadlineAt = myAccess?.PersonalDeadlineAt
        });
    }
    ...
    public async Task<IActionResult> Submit(AssignmentSubmitViewModel model)
    {
        ...
        var existingSubmission = await db.AssignmentSubmissions
        ...
        var access = await accessService.EnsureAccessAsync(assignment, userId);
        await accessService.RefreshStatusAsync(access, null);

        if (access.Status == AssignmentStudentStatus.Missing || DateTimeOffset.UtcNow > access.PersonalDeadlineAt)
        {
